...
} ngx_http_upstream_check_srv_conf_type;


static ngx_int_t ngx_http_check_add_timers(ngx_cycle_t *cycle);
...
static check_conf_t *ngx_http_get_check_type_conf(ngx_str_t *str);
...
ngx_http_check_recv_handler(ngx_event_t *event)
{
    ...
    while (1) {
        ...
        /* Not enough buffer? Enlarge double */
        if (n == 0) {
...
/* Configure and initialized funtions */
...
ngx_http_upstream_check_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    if (ucmcf->peers->peers.nelts > 0) {
        ...
        /* the default check share memory size is 1M */
